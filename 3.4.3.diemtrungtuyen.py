...
def chuyen_df(path):
    ...
    if diem_col is None or 'Mã ngành' not in df_all.columns:
        print(f"File loi thieu 'Mã ngành' hoac 'Điểm trúng tuyển': {path}")
        print(f"Cac cot hien co: {df_all.columns.tolist()}")
        return pd.DataFrame()  
    ...
    print(f"Da doc {path} - So dong: {len(df_all)} | Ma nganh: {df_all['Mã ngành'].unique().tolist()}")
    return df_all
...
def chuyen_df_pdfplumber(path):
    ...
    if not df.empty:
        print(f"[PLUMBER] {path} - {len(df)} dong | Ma nganh: {df['Mã ngành'].unique().tolist()}")
    else:
        print(f"Khong trich duoc du lieu tu: {path}")
    return df
...
# === MAIN ===
path_folder = '/home/nifi/HOCTAP/data/Diem_tuyen_sinh'
...
if list_df:
    ...
    output_path = '/home/nifi/HOCTAP/Do_thi_KQ_TS/do_thi_trung_binh_all_nam_theo_nganh.png'
